...
int main(int argc, char **argv)
{
    ...
    ("b", "Realiza leitura do blackboard sem movimentar o robo")
    ;
    ...
    write_int(mem, LIBERA_ACT, 0);
    write_int(mem, ACAO_EXECUTADA, 0);
    write_int(mem, RESET_ENV, 0);
    write_int(mem, RESP_RESET, 0);
    write_int(mem, RECOMPENSA, 0);
    write_int(mem, FINALIZADO, 0);
    write_int(mem, INICIO_SIMU, 1);
    ...
    //======================== read blackboard decision ================================     
    if (variables.count("b")) //realiza leitura da variavel DECISION_ACTION_A
    {
        int conta=0;
        while(1)
        {
            ...
            if(read_int(mem, LIBERA_ACT) == 1)
            {
                ...
                if(read_int(mem, DECISION_ACTION_A) == 3)
                    if(same_moviment == false)
                        std::cout<< " | \e[38;5;45mTurn right\e[0m" << std::endl;

                if(read_int(mem, DECISION_ACTION_A) == 4)
                    if(same_moviment == false)
                        std::cout<< " | \e[38;5;45mTurn left\e[0m"<<std::endl;

                if(read_int(mem, DECISION_ACTION_A) == 5)
                    if(same_moviment == false)
                        std::cout<< " | \e[38;5;45mKick_left_strong\e[0m";

                if(read_int(mem, DECISION_ACTION_A) == 6)
                    if(same_moviment == false)
                        std::cout<< " | \e[38;5;45mKick_right_strong\e[0m";
            }

            if( conta>30)
            {
                conta=0;
                //Informa que a acao ja foi executada
                std::cout<< "Decision = "<<read_int(mem, DECISION_ACTION_A)<<std::endl;
                write_int(mem, ACAO_EXECUTADA, 1);
            }
            conta++;
...
//            fflush (stdout);
    ...
    //Inicia o controle com o robo parado
    write_int(mem, DECISION_ACTION_A, 99);
