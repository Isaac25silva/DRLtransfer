    ...
    def on_step_begin(self, step, logs):
        if self.step % self.interval == 0:
            if len(self.episode_rewards) > 0:
                ...
                print('')
                string = str(self.step // self.interval )+" "+str(len(self.episode_rewards))+" "+str(np.mean(self.episode_rewards))+" "+str(np.min(self.episode_rewards))+" "+str(np.max(self.episode_rewards))+" "+str(self.step)+"\n"
                with open('reward_file.dat','a') as f:
                    f.write(string)
            self.reset()
