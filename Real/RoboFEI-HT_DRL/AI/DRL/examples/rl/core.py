...
class Env(object):
    ...
    def __init__(self, testlib, Mem, bkb):
    ...
    def step(self, action):
        ...
        #=======Realiza o reset depois do numero maximo de passos executados========
        self.step_count+=1
        if self.step_count >= 400:
            self.step_count = 0
            ob = self.reset()
            pointer = self.testlib.leitura_int()
            for x in range(100):
                if self.bkb.read_int(self.Mem,'INICIO_SIMU') == 1:
                    self.bkb.write_int(self.Mem,'INICIO_SIMU', 0) #inicio concluido
                    time.sleep(0.2)
                    return ob, 0, 1, {"status": "inicio"}
                    break
                time.sleep(0.05)
        #============================================================================

        self.bkb.write_int(self.Mem,'DECISION_ACTION_A', action)  #executa uma acao
        print action

        #Aguarda a acao ser executada========================
#        time.sleep(0.2)
        #Informa ao controle que a acao deve ser executada
        self.bkb.write_int(self.Mem,'LIBERA_ACT', 1) #self.testlib.escreve_int(1000008, 1)
#        pointer = self.testlib.leitura_int()
        #Aguarda o calculo da recompensa----
        while True:
            if self.bkb.read_int(self.Mem,'ACAO_EXECUTADA') == 1:
                self.bkb.write_int(self.Mem,'ACAO_EXECUTADA', 0)
                break
            time.sleep(0.001)
        #====================================================

        reward = self.bkb.read_int(self.Mem,'RECOMPENSA')*0.1 #verifica a recompensa
  
        final  = self.bkb.read_int(self.Mem,'FINALIZADO') #verifica se o episodio foi finalizado
        if final == 1:
            self.step_count = 0

        ob     = self._get_image()

        return ob, reward, final, {"status": "normal"}
        raise NotImplementedError()

    def _get_image(self):
        image_pointer = self.testlib.leitura_int()
    ...
    def reset(self):
        ...
        """
        self.bkb.write_int(self.Mem,'RESET_ENV', 1) #reset
        pointer = self.testlib.leitura_int()
        for x in range(500):
            if self.bkb.read_int(self.Mem,'RESP_RESET') == 1:
                self.bkb.write_int(self.Mem,'RESET_ENV', 0) #reset
                self.bkb.write_int(self.Mem,'RESP_RESET', 0) #confirma reset
                return self._get_image()
            time.sleep(0.001)
        print "error durante o Reset"
        return self._get_image()
