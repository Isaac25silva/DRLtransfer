...
class TrainIntervalLogger(Callback):
    def __init__(self, interval=10000):
        ...
        self.step = 0
        self.reset()
        with open('reward_file.dat','a') as f:
            pass
        self.testlib = ctypes.CDLL('/home/fei/Documents/Isaac_DRL/DRL_transfer/Simulator/transfer_DRL/examples/blackboard/blackboard.so') #chama a lybrary que contem as funções em c++
        self.testlib.using_shared_memory()   #usando a função do c++
        self.testlib.leitura_int.restype = ctypes.POINTER(ctypes.c_int) #define o tipo de retorno da função, neste caso a função retorna ponteiro int
    ...
    def on_step_begin(self, step, logs):
        if self.step % self.interval == 0:
            if len(self.episode_rewards) > 0:
                ...
                print('{} episodes - episode_reward: {:.3f} [min {:.3f}, max {:.3f}]{}{}'.format(len(self.episode_rewards), np.mean(self.episode_rewards), np.min(self.episode_rewards), np.max(self.episode_rewards), formatted_metrics, formatted_infos))
                print('')
                pointer = self.testlib.leitura_int()
                string = str(self.step // self.interval )+" "+str(len(self.episode_rewards))+" "+str(np.mean(self.episode_rewards))+" "+str(np.min(self.episode_rewards))+" "+str(np.max(self.episode_rewards))+" "+str(self.step)+" "+str(pointer[1000009])+" "+str(pointer[1000010])+" "+str(pointer[1000011])+"\n"
                pointer[1000009] = 0 #conta o numero de goals
		pointer[1000010] = 0 #conta quantas vezes alcancou a bola
                pointer[1000011] = 0 #conta quantas vezes falhou nos 400 steps
