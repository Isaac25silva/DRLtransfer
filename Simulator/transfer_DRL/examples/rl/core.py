...
class Env(object):
    ...
    def step(self, action):
        ...
        if self.step_count >= 200:
            ...
            ob = self.reset()
            pointer = self.testlib.leitura_int()
            pointer[1000011] = pointer[1000011] + 1 #incrementa quando falha
            for x in range(100):
                if pointer[1000006] == 1:
                    self.testlib.escreve_int(1000006, 0) #inicio concluido
                    time.sleep(0.2)
        ...
        self.testlib.escreve_int(1000000, action)  #executa uma acao

        #Aguarda a acao ser executada========================
#        time.sleep(0.2)
        #Informa ao controle que a acao deve ser executada
        self.testlib.escreve_int(1000008, 1)  
        pointer = self.testlib.leitura_int()
        #Aguarda o calculo da recompensa----
        while True:
            if pointer[1000008] == 0:
                break
            time.sleep(0.001)
        #====================================================

        reward = pointer[1000003]*0.1 #verifica a recompensa
  
        final  = pointer[1000004] #verifica se o episodio foi finalizado
        if final is not 0:
            self.step_count = 0
            if reward > 0.999:
                pointer[1000009] = pointer[1000009] + 1 #incrementa quando faz goal
    ...
    def reset(self):
        ...
        """
        self.testlib.escreve_int(1000001, 1) #reset
        pointer = self.testlib.leitura_int()
        for x in range(50):
            if pointer[1000002] == 1:
                self.testlib.escreve_int(1000001, 0) #reset
                self.testlib.escreve_int(1000002, 0) #confirma reset
